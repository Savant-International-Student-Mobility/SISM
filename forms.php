...
// PIGILAN ANG BROWSER NA I-SAVE ANG PAGE SA CACHE (Anti-back button bypass)
...
// Kunin ang First Name ng naka-login na user para sa Header Badge
...
<head>
  ...
  <style>
    ...
    .form-card { transition: transform 0.2s; } .form-card:hover { transform: translateY(-5px); }
    .btn-form-action { color: var(--savant-primary-green); border-color: var(--savant-primary-green); }
    .btn-form-action:hover { background-color: var(--savant-primary-green); color: white; }
    ...
    @media (max-width: 992px) { body { padding-top: 60px; padding-bottom: 50px; overflow: auto; } .main-wrapper { flex-direction: column; height: auto; } .content-area { height: auto; overflow-y: visible; margin-left: 0; } .header { position: fixed; top: 0; left: 0; right: 0; z-index: 1030; height: 60px; } .header-title { display: none; } .sidebar { width: 100%; height: 50px; position: fixed; bottom: 0; left: 0; z-index: 1029; flex-direction: row; align-items: center; padding: 0; } .sidebar .logo, .sidebar .footer-text { display: none; } .sidebar .nav { display: flex; flex-direction: row; width: 100%; justify-content: space-around; } .sidebar .nav-link span { display: none; } .chat-toggle-btn { bottom: 80px; right: 15px; } .chat-container { display: none !important; } }
  </style>
</head>
      ...
      <main class="main-content" style="color: #087830">
        ...
        <div class="row g-4">
          <div class="col-12 col-md-6 col-lg-4">
            <div class="card form-card h-100 shadow-sm border-0">
              <div class="card-body">
                <h5 class="card-title fw-bold">Application Form</h5>
                <p class="card-text text-muted">Complete this form and apply in our services.</p>
                <a href="applicationform.php" class="btn btn-sm btn-outline-secondary btn-form-action px-3">Fill Out</a>
              </div>
            </div>
          </div>
        </div>
